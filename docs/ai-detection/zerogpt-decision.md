...
> **Этот документ содержит исследование ZeroGPT. Решение: ZeroGPT отклонён в пользу GPTZero (см. gptzero-decision.md).**
> Фактические данные (API, цены, accuracy) оставлены для справки. Рекомендации ниже отражают историческое решение задачи 009 и **не актуальны** — финальный выбор сделан в gptzero-decision.md.
...
## ZeroGPT ОТКЛОНЁН — GPTZero выбран вместо него

> Первоначальное исследование (ниже) рассматривало ZeroGPT как стартовый инструмент. После сравнительного анализа (задача 010) ZeroGPT отклонён: Trustpilot 1.3/5, задокументированные false negatives (ChatGPT-текст = 0% AI), ненадёжен даже для калибровки. GPTZero выбран как основной AI-детектор с первого дня.

### Историческое обоснование ZeroGPT как дополнения к Ahrefs (не актуально)
...
**Актуальное решение:** ZeroGPT отклонён. GPTZero выбран с первого дня (API ключ получен от Егора, 300K слов/мес). Originality.ai остаётся запасным вариантом если GPTZero окажется недостаточно точным.
